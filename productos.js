...
let productosJSON = JSON.parse(fs.readFileSync('./productos.json', 'utf-8'));
// Usar el método readFileSync para leer el archivo .json 
// parsear el json y guardarlo en una variable
...
module.exports = moduloProductos = { // Declaro el módulo productos como un objeto literal que contiene muchos métodos.
    ...
    leerJSON: function(){ // El primer método del módulo se encargará de:
        return productosJSON // el método retorna un Array de objetos literales
    ...
    escribirJSON: (nombreProducto, precio)=>{ // Método que se encargará de agregar un nuevo producto al .json y recibe dos parámetros
        ...
        productosJSON.map(function(producto){ // recorro con el método .map nuestro array de objetos.
        ...
        productosJSON.push(nuevoProducto); // usando el método .push agrego al nuestra lista el nuevo producto.

        let productosActualizados = JSON.stringify(productosJSON); // con el método stringify convierto la lista a una cadena de texto tipo json 
    ...
    filtrarJSON: (precioMinimo, precioMaximo )=>{ // Método que filtra la lista de productos según dos parámetros
        ...
        let listaFiltrada = productosJSON.filter((producto)=>{ // declaro una variable que almacenará lo que retorna el método .filter (todos los productos que cumplen con los parámetros solicitados).
    ...
    cambiarPrecio: (id, nuevoPrecio)=>{ // Método para cambiar el precio de el producto con id igual al pasado por parámetro.
        ...
        let productoAModificar = productosJSON.filter((producto)=>{ // aplico el método .filter a la lista de productos.
            ...
            return productosJSON // retorna la lista de productos ya modificada.
        })
    ...
    eliminar : (id)=>{ // Método que elimina el producto cuyo id coincida con el parámetro enviado.
        ...
        let quitarProducto = productosJSON.filter(producto =>{ 
    ...
    buscar : (busqueda)=>{ // Método que busca los productos cuyo nombre coincidan con el argumento dado.
        ...
        let productoBuscado = productosJSON.filter(producto => {
